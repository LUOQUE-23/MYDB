...
- [X] CLI starts and supports `mydb --version`
- [X] `cargo fmt --all`
...
- [X] Interactive shell / protocol (minimal REPL via `mydb shell`)
- [X] Instance bootstrap and remote CLI flow (`mydb init/start/connect` with host/port/admin password config)
- [X] Service operations UX (`mydb status` + authenticated `mydb stop` graceful shutdown)
