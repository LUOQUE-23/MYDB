...
use mydb_core::{DataType, MyDbError, Result, Value};

use crate::Rid;

const MAGIC: &[u8; 8] = b"MYDBBTI1";
...
impl IndexKeyKind {
    pub fn from_data_type(data_type: DataType) -> Result<Self> {
        match data_type {
            ...
            DataType::Double => Err(MyDbError::Parse(
    ...
    fn from_u8(raw: u8) -> Result<Self> {
        match raw {
            ...
            2 => Ok(Self::Str),
            3 => Ok(Self::Bool),
            _ => Err(MyDbError::Corruption(format!(
        ...
        match self.read_node(page_id)? {
            Node::Leaf(mut leaf) => {
                ...
                if leaf.entries.len() < 2 {
                    return Err(MyDbError::RecordTooLarge {
                ...
                let separator = right_entries.first().map(|entry| entry.key.clone()).ok_or(
                    MyDbError::Corruption("leaf split generated empty right node".to_string()),
                )?;
            ...
            Node::Internal(mut internal) => {
                ...
                let separator_entry = internal.keys.pop().ok_or(MyDbError::Corruption(
                ...
                Node::Internal(mut internal) => {
                    if let Some(left_page_id) = left_sibling_page {
                        ...
                        if left_internal.keys.len() > 1 {
                            ...
                            let borrowed =
                                left_internal.keys.pop().ok_or(MyDbError::Corruption(
...
fn encoded_key_len(key: &IndexKey, key_kind: IndexKeyKind) -> Result<usize> {
    match (key_kind, key) {
        ...
        (IndexKeyKind::Str, IndexKey::Str(v)) => {
            let len = v.len();
            let _ = u16::try_from(len).map_err(|_| MyDbError::RecordTooLarge { size: len })?;
            Ok(2 + len)
        }
        _ => Err(MyDbError::Corruption(
    ...
    match (key_kind, key) {
        ...
        (IndexKeyKind::Str, IndexKey::Str(v)) => {
            ...
            let len = u16::try_from(bytes.len())
                .map_err(|_| MyDbError::RecordTooLarge { size: bytes.len() })?;
    ...
    Ok(match key_kind {
        ...
        IndexKeyKind::Str => {
            ...
            let value = String::from_utf8(bytes.to_vec()).map_err(|err| {
                MyDbError::Corruption(format!("invalid utf8 in index key: {err}"))
            })?;
            IndexKey::Str(value)
        }
    })
...
fn encode_leaf(leaf: &LeafNode, key_kind: IndexKeyKind) -> Result<[u8; PAGE_SIZE]> {
    ...
    if needed > PAGE_SIZE {
        return Err(MyDbError::RecordTooLarge { size: needed });
    }
