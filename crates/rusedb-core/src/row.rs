...
use crate::{DataType, Result, RuseDbError, Schema, Value};
...
impl Row {
    ...
    pub fn encode(&self, schema: &Schema) -> Result<Vec<u8>> {
        ...
        for (index, (column, value)) in schema.columns.iter().zip(&self.values).enumerate() {
            if matches!(value, Value::Null) {
                if !column.nullable {
                    return Err(RuseDbError::NullConstraintViolation {
            ...
            match (column.data_type, value) {
                ...
                (DataType::Varchar, Value::Varchar(v)) => {
                    ...
                    if start > u16::MAX as usize || len > u16::MAX as usize {
                        return Err(RuseDbError::RecordTooLarge { size: start + len });
                    }
        ...
        let fixed_len_u16 =
            u16::try_from(fixed_area.len()).map_err(|_| RuseDbError::RecordTooLarge {
                size: fixed_area.len(),
            })?;
        let null_len_u16 =
            u16::try_from(null_bitmap_len).map_err(|_| RuseDbError::RecordTooLarge {
                size: null_bitmap_len,
            })?;
        let var_count_u16 =
            u16::try_from(var_offsets.len()).map_err(|_| RuseDbError::RecordTooLarge {
    ...
    pub fn decode(schema: &Schema, bytes: &[u8]) -> Result<Self> {
        if bytes.len() < HEADER_SIZE {
            return Err(RuseDbError::Corruption("row payload too short".to_string()));
        }
        ...
        if bytes.len() < var_data_start {
            return Err(RuseDbError::Corruption("row payload truncated".to_string()));
        }
                ...
                DataType::Varchar => {
                    ...
                    if is_null {
                        ...
                        let s = str::from_utf8(&var_data[offset..end]).map_err(|err| {
                            RuseDbError::Corruption(format!("invalid utf8 varchar payload: {err}"))
                        })?;
